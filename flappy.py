...
#bird_surface = pygame.image.load('assets/bluebird-midflap.png').convert_alpha()  #to remove black square on  surface
#bird_rect = bird_surface.get_rect(center =(50,256))
#scale image to screen dimension
#bg_surface =pygme.transform.scale2x(bg_surface) to double
...
while True:
    for event in pygame.event.get():
        ...
        if event.type== pygame.KEYDOWN :
            ...
            if event.key == pygame.K_SPACE and game_active==False: #restrat the game
                game_active = True
                pipe_list.clear()
        ...
        if event.type == SPAWNPIPE:
            pipe_list.extend(create_pipe())  #calling pipe function
